...
function initBottomNavigation() {
  ...
  bottomNav.innerHTML = `
    <div class="bottom-nav-wrapper">
      ...
      <button class="bottom-nav-item" data-page="progress"><span class="bottom-nav-icon">📈</span><span>Progress</span></button>
      <button class="bottom-nav-item" data-page="customize"><span class="bottom-nav-icon">➕</span><span>Add Habit</span></button>
      <button class="bottom-nav-item" data-page="daily-tracker"><span class="bottom-nav-icon">👤</span><span>Profile</span></button>
    </div>
  `;
...
function initLoginPage() {
  ...
  form?.addEventListener('submit', function (e) {
    ...
    if (!email || !terms) {
      showToast('Complete the form', 'Email and confirmation are required.', 'error');
      return;
    }
  ...
  verifyBtn?.addEventListener('click', function () {
    ...
    setTimeout(() => {
      ...
      showToast('Welcome back', 'You are now signed in to HabitFlow.', 'success');
      navigateTo('index');
    }, 900);
  });
  ...
  googleBtn?.addEventListener('click', function () {
    ...
    setTimeout(() => {
      showLoading(false);
      localStorage.setItem('userLoggedIn', 'true');
      showToast('Signed in with Google', 'Welcome back to HabitFlow.', 'success');
      navigateTo('index');
    }, 900);
  });
}
...
function initSplashPage() {
  ...
  const title = document.getElementById('splash-headline');
  const subtitle = document.getElementById('splash-copy');
...
function initApp() {
  ...
  if (!loggedIn) {
    ...
    if (seenSplash && !seenOnboarding && !currentPage.startsWith('onboarding') && currentPage !== 'splash') {
